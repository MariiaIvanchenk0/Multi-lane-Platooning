...
The integrated pose is broadcasted on /tf so it can be visualised directly in RViz.
"""
...
class ModelSimulationNode(Node):
    ...
    def frenet_to_cartesian(self, state):
        ...
        global_v = math.sqrt((denominator * v_long) ** 2 + v_lat ** 2)
        
        return x, y, global_theta, global_v

    def publish_to_sim(self, state):
        x, y, theta, v = self.frenet_to_cartesian(state)
